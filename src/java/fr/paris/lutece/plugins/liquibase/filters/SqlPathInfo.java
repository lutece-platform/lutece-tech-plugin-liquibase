...
public class SqlPathInfo
{
    ...
    private static final Pattern SQL_CREATE_PATTERN = Pattern.compile("sql/plugins/(?<plugin>[\\p{Alnum}]+)/(core|plugin)/(init|create)[\\p{Alnum}_]+\\.sql");
    ...
    private static final Pattern SQL_UPDATE_PATTERN = Pattern.compile(
            "sql/plugins/(?<plugin>[\\p{Alnum}]+)/upgrade/update[\\p{Alnum}_]+[\\-_]?(?<srcVersion>([0-9]+(\\.[0-9]+)*))[\\-_](?<dstVersion>([0-9]+(\\.[0-9]+)*))\\.sql");

    // matches src/sql/create_db_lutece_core.sql and src/sql/init_db_lutece_core.sql
    private static final Pattern SQL_CORE_CREATE = Pattern.compile("sql/(init|create)[\\p{Alnum}_]+(?<plugin>core)\\.sql");
